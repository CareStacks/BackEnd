...
The backend was initialized as a Spring Boot REST API for the CareConnect platform.
...
- PostgreSQL driver for the relational database defined in the architecture
- Springdoc OpenAPI 3.0.3 for API documentation
- Spring Boot DevTools for local development

**Implemented bounded contexts:**

- `notifications`: manages notification scheduling, listing, read state, alerts and notification preferences.
- `agenda`: manages health events, reminders, date validation, conflict detection, confirmation, rescheduling and cancellation.

**Branch workflow used:**

- `develop` was created from `main`.
- `feature/notifications` was created from `develop` and merged back into `develop`.
- `feature/agenda` was created from `develop` and merged back into `develop`.
- Final integration is performed from `develop` into `main`.
...
The default profile uses H2 in-memory storage. To use PostgreSQL, run with the `postgres` profile and provide the database environment variables:

```bash
SPRING_PROFILES_ACTIVE=postgres \
DB_URL=jdbc:postgresql://localhost:5432/careconnect \
DB_USERNAME=careconnect \
DB_PASSWORD=careconnect \
mvn spring-boot:run
```
...
- `PATCH /api/notifications/read-all?recipientId={recipientId}`
- `PATCH /api/notifications/{id}/cancel`
- `DELETE /api/notifications/{id}`
- `POST /api/notifications/alerts`
- `GET /api/notifications/alerts/active?recipientId={recipientId}`
- `PATCH /api/notifications/alerts/{id}/resolve`
- `GET /api/notifications/preferences/{recipientId}`
- `PUT /api/notifications/preferences/{recipientId}`

**Main Agenda endpoints:**

- `POST /api/agenda`
- `GET /api/agenda`
- `GET /api/agenda/{id}`
- `GET /api/agenda/patient/{patientId}`
- `GET /api/agenda/date?patientId={patientId}&date={yyyy-MM-dd}`
- `PUT /api/agenda/{id}`
